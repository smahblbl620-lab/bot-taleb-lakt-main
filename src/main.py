...
from telethon.errors import SessionPasswordNeededError, PhoneCodeInvalidError, PhoneCodeExpiredError, FloodWaitError, PhoneNumberFloodError, PhoneNumberInvalidError, PasswordHashInvalidError
...
async def setup_bot_handlers():
    ...
    async def callback_handler(event):
        ...
        if data == b'add_acc':
            ...
            logger.info(f"➕ المستخدم {user_id} بدأ عملية إضافة حساب (زر الإضافة)")
            quota_line = ""
            ...
            await event.respond(
                ...
                "🔑 **شرط وصول الكود في تيليجرام (مثل السابق تماماً):** الحساب المُضاف لازم يكون **مفتوحاً في تطبيق تيليجرام في جهازك** قبل ما ترسل الرقم — الكود يصل دائماً في محادثة **Telegram** الرسمية داخل الحساب المُضاف نفسه، وبعدين تنسخه وتلصقه هنا.\n\n"
    ...
    async def input_handler(event):
        ...
        if user_id not in login_states:
            # 🛡 منع الصمت: مستخدم مصرح له يرسل رقم هاتف بلا عملية جارية (غالباً بعد إعادة نشر فقدت الحالة) — نوجّهه للزر بدل التجاهل التام
            try:
                ...
                if _phone_like and event.is_private and is_admin(user_id):
                    ...
                    logger.info(f"🛡 المستخدم {user_id} أرسل رقماً بلا حالة تسجيل — وُجّه للزر")
            except Exception:
                pass
            return
        ...
        # إضافة حساب - رقم الهاتف
            ...
            # 3) ⚡ جلسة قديمة مصرّحاً بها على القرص (الحساب أُضيف سابقاً) — ربط فوري بدون أي كود
            _sess_file = os.path.join(SESSION_DIR, f'session_{phone}.session')
            if os.path.exists(_sess_file):
                _rec = None
                try:
                    _rec = TelegramClient(os.path.join(SESSION_DIR, f'session_{phone}'), API_ID, API_HASH, **CLIENT_OPTS)
                    await _rec.connect()
                    if await _rec.is_user_authorized():
                        _own_now = config.get('ACCOUNT_OWNERS', {}).get(phone)
                        if _own_now and str(_own_now) != str(user_id) and not is_full_admin(user_id):
                            await _rec.disconnect()
                            del login_states[user_id]
                            await event.respond("🚫 هذا الحساب مسجل لدى مستخدم آخر — لا يمكنك إضافته.")
                            return
                        await event.respond(f"⚡ الحساب `{phone}` عنده جلسة سابقة مصرّحاً بها — تم ربطه **بدون كود تحقق**.")
                        new_count = await import_groups(_rec)
                        await event.respond(f"📦 تم استيراد `{new_count}` مجموعة (المراقبة تشمل جميع المجموعات).")
                        asyncio.create_task(export_group_links(_rec, phone))
                        active_clients[phone] = _rec
                        set_account_owner(phone, state.get('owner') or user_id)
                        save_session_string(phone, _rec)
                        register_handler(_rec, phone)
                        asyncio.create_task(start_monitoring(_rec, phone))
                        del login_states[user_id]
                        logger.info(f"⚡ استرداد فوري بلا كود للحساب {phone} من جلسة القرص (بطلب المستخدم {user_id})")
                        return
                    await _rec.disconnect()
                    _rec = None
                    logger.info(f"ℹ️ جلسة قرص قديمة غير مصرح بها لـ {phone} — سيُطلب كود جديد")
                except Exception as _re:
                    logger.warning(f"⚠️ فحص جلسة القرص لـ {phone} فشل: {str(_re)[:120]}")
                    try:
                        if _rec is not None:
                            await _rec.disconnect()
                    except Exception:
            ...
            try:
                logger.info(f"📩 طلب كود تحقق لـ {phone} (بطلب المستخدم {user_id})")
                sent_code = await new_client.send_code_request(phone)
                _hash = getattr(sent_code, 'phone_code_hash', None)
                if not _hash:
                    logger.error(f"❌ تيليجرام لم يُرجع phone_code_hash لـ {phone}")
                    raise RuntimeError("لم يستلم البوت معرّف الكود من تيليجرام — انتظر دقيقة وأعد المحاولة من الزر.")
                _tname = type(getattr(sent_code, 'type', None)).__name__
                if 'App' in _tname:
                    _ch = 'تطبيق تيليجرام'
                    _hint = (
                        f"الكود وصل **داخل تطبيق تيليجرام للحساب `{phone}` نفسه** — ليس SMS ولا مكالمة:\n"
                        "1️⃣ افتح تطبيق تيليجرام وبدّل إلى **الحساب المُضاف نفسه** (قائمة الحسابات في أعلى التطبيق)\n"
                        "2️⃣ افتح محادثة **Telegram** الرسمية (الحساب الموثّق) — ستجد فيها رسالة فيها الكود\n"
                        "3️⃣ انسخ الكود وألصقه هنا فوراً\n"
                        "⚠️ إن لم تجد الحساب في تطبيق جهازك فمعناها الحساب مفتوح في جهاز آخر — الكود وصل هناك. افتح الحساب في جهازك أولاً ثم أعد الإضافة."
                    )
                elif 'Sms' in _tname:
                    _ch = 'رسالة SMS'
                    _hint = (
                        "تيليجرام أرسل الكود **SMS على الرقم** — هذا قرار مباشر من تيليجرام لأن الحساب غير مفتوح في أي جهاز الآن، ولا يستطيع أي بوت تغييره.\n\n"
                        "🔑 **لجعل الكود يأتي في تيليجرام دائماً بعد اليوم:**\n"
                        "1️⃣ افتح تطبيق تيليجرام في جهازك وأضف هذا الحساب كحساب جديد في التطبيق (تيليجرام قد يرسل SMS مرة واحدة فقط لتسجيلك في التطبيق)\n"
                        "2️⃣ بعد أن يصير الحساب مفتوحاً في تطبيق جهازك، أضفه للبوت — وسيصلك الكود **داخل تيليجرام** كل مرة بعدها."
                    )
                elif 'Call' in _tname or 'Flash' in _tname:
                    _ch = 'مكالمة هاتفية'
                    _hint = (
                        "تيليجرام سيتصل **على الرقم نفسه** وينطق الكود صوتياً — لأن الحساب غير مفتوح في أي جهاز (قرار مباشر من تيليجرام لا يمكن لأي بوت تغييره).\n\n"
                        "🔑 **لجعل الكود يأتي في تيليجرام دائماً بعد اليوم:** افتح تطبيق تيليجرام في جهازك وأضف هذا الحساب كحساب جديد في التطبيق، وبعد أن يصير مفتوحاً في جهازك أضفه للبوت — وسيصلك الكود **داخل تيليجرام** كل مرة بعدها."
                    )
                else:
                    _ch, _hint = 'غير محددة', "افحص تطبيق تيليجرام للحساب المُضاف (بدّل إلى الحساب نفسه ← محادثة **Telegram** الرسمية) ورسائل الرقم."
                logger.info(f"✅ أُرسل كود {phone} | القناة: {_tname} | hash موجود")
                login_states[user_id] = {'step': 'await_code', 'phone': phone, 'hash': _hash, 'client': new_client, 'owner': state.get('owner', user_id), 'last_code_req': time.time()}
                await event.respond(
                    f"📩 تم إرسال الكود إلى `{phone}` — قناة الإرسال: **{_ch}**.\n\n"
                    f"📥 {_hint}\n\n"
        ...
        # إضافة حساب - رمز التحقق
            ...
            try:
                ...
                if not client.is_connected():
                    await client.connect()
                logger.info(f"🔢 استلام كود من المستخدم {user_id} لـ {state['phone']} (طول {len(text)}) — محاولة sign_in")
                await client.sign_in(state['phone'], text, phone_code_hash=state['hash'])
                logger.info(f"✅ sign_in نجح للحساب {state['phone']} (المستخدم {user_id})")
                ...
                logger.info(f"🔐 الحساب {state['phone']} محمي بـ2FA — انتظار كلمة السر")
                ...
                logger.warning(f"❌ كود خاطئ من المستخدم {user_id} لـ {state['phone']}: «{text}»")
                await event.respond(
                    "❌ **الكود غير صحيح.**\n\n"
                ...
                logger.error(f"❌ فشل sign_in لـ {state['phone']} (المستخدم {user_id}): {type(e).__name__}: {str(e)[:200]}")
                await event.respond(f"❌ خطأ: {e}"); del login_states[user_id]
        ...
        # إضافة حساب - كلمة المرور (2FA)
            ...
            try:
                ...
                if not client.is_connected():
                    await client.connect()
                logger.info(f"🔐 استلام كلمة سر 2FA من المستخدم {user_id} للحساب {state['phone']} — محاولة sign_in")
                await client.sign_in(password=text)
                logger.info(f"✅ sign_in (2FA) نجح للحساب {state['phone']} (المستخدم {user_id})")
                ...
                # ❌ كلمة السر خاطئة — السماح بإعادة المحاولة فوراً بدون إلغاء العملية
                logger.warning(f"❌ كلمة سر 2FA خاطئة من المستخدم {user_id} للحساب {state['phone']} — سُمح بإعادة المحاولة")
                await event.respond(
                    "❌ **كلمة السر غير صحيحة.**\n\n"
                    "🔑 أعد إرسال كلمة السر الصحيحة الآن — أو أرسل `/cancel` للإلغاء."
                )
            except Exception as e:
                logger.error(f"❌ فشل sign_in بكلمة السر لـ {state['phone']} (المستخدم {user_id}): {type(e).__name__}: {str(e)[:200]}")
                await event.respond(f"❌ خطأ: {e}"); del login_states[user_id]
